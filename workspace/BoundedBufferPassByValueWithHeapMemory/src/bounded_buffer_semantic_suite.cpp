...
void test_buffer_push_copies_element() {
	...
	MemoryOperationCounter counter { }, expected { 1, 1, true }; // pass-by-value, extra move
	buffer.push(counter);
...
void test_element_in_buffer_is_copied_along_copy_constructor() {
	BoundedBuffer<MemoryOperationCounter> buffer{ 1 };
	MemoryOperationCounter counter{}, expected{2, 2, true}; // pass-by-value, extra move
	buffer.push(counter);
...
void test_element_in_buffer_is_moved_along_move_constructor() {
	BoundedBuffer<MemoryOperationCounter> buffer{1};
	MemoryOperationCounter counter{}, expected{2, 0, true};// pass-by-value, extra move
	buffer.push(std::move(counter));
...
void test_element_in_buffer_is_copied_along_assignment_operator_multiple_copies() {
	...
	MemoryOperationCounter counter{}, expected{4, 4, true};// pass-by-value, extra move
	buffer.push(counter);
