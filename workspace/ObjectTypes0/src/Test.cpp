...
using sean::circle;
struct composite;
using widget = std::variant<rect,circle,composite,int,std::string>;
using widgets=std::vector<widget>;
...
// draw overloads and other value classes as before
// helper for visitor, did not make it into C++17/20
...
void draw(widget const &w, screen &on) {
// must provide all overloads for variant members
  visit( overloaded {
    [&on](int const &i) { draw(i, on);},
    [&on](std::string const &s) { draw(s, on);},
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(variantwidget::testRect));
	s.push_back(CUTE(variantwidget::testCircle));
	s.push_back(CUTE(variantwidget::testComposite));
