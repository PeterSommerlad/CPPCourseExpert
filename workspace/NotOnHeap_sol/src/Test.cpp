...
void test_with_make_shared() {
	auto const sp = std::make_shared<not_on_heap>();
	ASSERTM("should be real pointer", sp != nullptr);
	//ASSERT_THROWS(std::make_shared<not_on_heap>(), std::bad_alloc); // cannot prevent
}

//Allocation with placement new
void test_with_placement_new() {
	auto const spaceForNotOnHeap = std::make_unique< std::byte[]> (sizeof(not_on_heap));
    //auto const ptr = ::new (spaceForNotOnHeap.get()) not_on_heap { };
    auto const ptr = reinterpret_cast<not_on_heap*>( spaceForNotOnHeap.get());
    //ASSERT(not ptr->valid()); // UB!
    std::construct_at( ptr );
	ASSERT(ptr->valid());
	std::destroy_at(ptr);
}
void test_with_vector_still_works(){
	std::vector<not_on_heap> v(2);
	ASSERT(v.at(1).valid());
}
...
void runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(test_with_vector_still_works));
